...
void fresnel(const Vec3f &I, const Vec3f &N, const float &ior, float &kr)
{
	...
	float sint = etai / etat * sqrtf(std::fmax(0.f, 1 - cosi * cosi));
	// Total internal reflection
	if (sint >= 1) {
		kr = 1;
	}
	else {
		float cost = sqrtf(std::fmax(0.f, 1 - sint * sint));
		cosi = fabsf(cosi);
	...
	if (trace(orig, dir, objects, tnear, index, uv, &hitObject)) {
		...
		default:
		{
			...
			for (uint32_t i = 0; i < lights.size(); ++i) {
				...
				float LdotN = std::fmax(0.f, dotProduct(lightDir, N));
				...
				specularColor += powf(std::fmax(0.f, -dotProduct(reflectionDirection, dir)), hitObject->specularExponent) * lights[i]->intensity;
			}
